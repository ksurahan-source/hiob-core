...
def sync_clips_from_slots(client: Client, run_id: str) -> dict:
    ...
    # Index null clips by (beat_index, clip_kind) → clip for O(1) deterministic
    # slot→clip pairing.  Eliminates the scan-and-mark loop that could mismatch
    # beats when Supabase returns slots in a non-beat order.
    clip_by_beat_kind: dict[tuple[int, str], dict] = {}
    clip_by_ms_kind: dict[tuple[int, str], dict] = {}
    for clip in null_clips:
        ...
        if bi is not None:
            clip_by_beat_kind[(int(bi), kind)] = clip
        else:
            ms = int(clip.get("start_ms") or 0)
            if (ms, kind) not in clip_by_ms_kind:
                clip_by_ms_kind[(ms, kind)] = clip

    # 6. Match each slot to its clip via (beat_index, clip_kind); patch artifact_id.
    updated = 0
    for slot in slot_rows:
        ...
        if beat_index is not None:
            target_clip = clip_by_beat_kind.pop((int(beat_index), clip_kind), None)
        else:
            # No beat_index: match by start_ms (music full-clip, sfx non-beat slots)
            target_clip = clip_by_ms_kind.pop((start_ms, clip_kind), None)
